...
- 函数基础总结：
  ...
  - 函数是声明一次但可以调用任意多次的一段代码，通过将要实现的功能，使用函数封装起来，实现代码复用，提示开发效率
...
- 函数其他说明
  ...
  - 函数按照顺序一行行的执行代码，当遇到调用其他函数时，先完成其他函数调用，在继续执行后面的代码

### 对象

对象是由特征和行为组成的，即：属性(特征)和方法(行为)

<font color='bronze'>对象是一组组相关属性和方法集合，并且是无序的</font>

如果要描述一个事物或一组相关数据，就可以使用对象来实现

语法：对象用花括号{ } 来表示，对象中的属性或方法采用的是键值对的形式，键值对之间用逗号分隔

对象中键值对的值是函数时，函数就被称为方法，函数用作方法时，可以省略function后面的函数名称，也叫匿名函数

函数和方法的区别：如果一个函数是单独出现的，没有和对象关联，就称之为函数；如果一个函数是对象里面某个键值对里的值，就称之为方法
...
let person = {
    ...
    age: 20,
    height: 175,
    sing: function(){ //sing是对象person的方法，调用该方法：person.sing()
...
- 对象属性的类型注解，语法：
  ...
  注意：对象类型注解里键值对中的**值是类型**，多个键值对之间使用分号分隔
  ...
  在使用对象前，可根据需要，提前设计好对象的结构，对象的结构化类型即类型注解，就是建立一种契约，约束对象的结构

- 对象方法的类型注解:
  
  箭头 => 左边小括号中的内容：表示方法的参数类型
  
  箭头 => 右边的内容：表示方法的返回值类型，如果方法没有返回值，则用void表示
  ...
  **方法类型注解的关键点：参数和返回值**
  ...
  person = {
      ...
      sun: function(num1:number,num2:number){
         return num1 + num2
     }
  }
  ```
...
接口就是为对象的类型注解进行命名，并为代码建立契约来约束对象的结构

实际上接口的作用和直接在对象名称后面写类型注解是一样的，但是接口为对象的类型注解命名，即对其取了一个名字，这样我们就可以把对象的类型注解作为一块内容单独出现，并对块内容取一个名称，需要使用类型注解或接口的时候，只需通过该名称就可以引用对象的类型注解接口了，这样使用就更方便了
...
```ts
// 为对象IUser定义了接口作为其类型注解
interface IUser {
    name: string,
    age: number
}

// 声明了一个变量p1，直接用接口IUser作为p1的类型
let p1:IUser = {
    name: 'Jack',
    age:18
}
